...
Object.assign(PIECES, {
  ...
  felguard: {
    ...
    stats: { ...CRIT_BASE, count: 1, respawn: 6, damage: 88, duration: 20,
             range: 210, attackInterval: 0.8, cleave: 80, guard: 20 },
    trigger: { type: "autonomous", count: "@count", interval: "@respawn" },
    effects: [
      { type: "summon", kind: "felguard", ai: "anchor", count: 1, cap: "@count", big: true,
        ...
        onHit: [{ type: "damage_instant", amount: "@damage*0.6", radius: "@cleave" },
                { type: "shield", amount: "@guard", cap: 220 }] },
    ],
    paths: {
      ...
      mastery: MASTERY({ dmg: ["damage", "guard"], noun: "dano e escudo",
                         evolvesInto: "grimoireOfSacrifice" },
      ...
      crit: CRIT({},
        T("Carnificina", "Todo golpe crita, atordoa por 0.5s e empurra.",
          { crit: { set: 1 } },
          { "effects.0.onHit.2": { type: "stun", duration: 0.5, radius: "@cleave" },
            "effects.0.onHit.3": { type: "knockback", force: 60, radius: "@cleave" } })),
    },
  },
