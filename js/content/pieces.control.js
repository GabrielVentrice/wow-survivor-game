...
Object.assign(PIECES, {
  ...
  curseOfExhaustion: {
    ...
    paths: {
      ...
      mastery: MASTERY({ dmg: "damage",
                         also: { stat: "factor", noun: "velocidade dos afetados",
                                 steps: [-0.1, -0.1, -0.12, -0.12], pct: true } },
        T("Estase", "Reduz a velocidade deles a um décimo.", { factor: { set: 0.1 } })),
      crit: CRIT({},
        T("Colapso", "Todo pulso crita, e a aura apodrece quem está dentro.",
          { crit: { set: 1 } },
          { "effects.2": { type: "damage_over_time", key: "exhaust", dps: "@damage*3",
  ...
  curseOfTongues: {
    ...
    paths: {
      haste: HASTE({ rate: { stat: "interval", verb: "Pulsa" } },
        T("Horizonte", "Dobra o raio da aura.", { radius: { mul: 2 } })),
      mastery: MASTERY({ dmg: "damage",
                         also: { stat: "factor", noun: "dano dos afetados",
                                 steps: [-0.1, -0.1, -0.12, -0.12], pct: true } },
      ...
      crit: CRIT({},
        T("Loucura", "Todo pulso crita e amedronta por 2s, chefes incluídos.",
          { crit: { set: 1 } },
          { "effects.2": { type: "fear", duration: 2, radius: "@radius",
                           affectsBoss: true } })),
    },
  },
  ...
  howlOfTerror: {
    ...
    stats: { ...CRIT_BASE, radius: 200, duration: 2.2, cooldown: 4, blast: 60 },
    trigger: { type: "reactive", event: "player_damaged", cooldown: "@cooldown", atPlayer: true },
    effects: [
      { type: "fear", duration: "@duration", radius: "@radius" },
      { type: "damage_instant", amount: "@blast", radius: "@radius", shape: "nova" },
    ],
    paths: {
      haste: HASTE({ rate: { stat: "cooldown", verb: "Grita" } },
        T("Horizonte", "Dobra o raio do grito.", { radius: { mul: 2 } })),
      mastery: MASTERY({ dmg: "blast", noun: "dano do grito" },
        T("Grito de Ruína", "Triplica o dano e o grito vira um golpe grande.",
          { blast: { mul: 3 } }, { "effects.1.big": true })),
  ...
  mortalCoil: {
    ...
    paths: {
      ...
      mastery: MASTERY({ dmg: ["blast", "heal"], noun: "dano e cura" },
        T("Vórtice", "O empurrão também atordoa por 1.5s.", null,
          { "effects.3": { type: "stun", duration: 1.5, radius: "@radius" } })),
      ...
      crit: CRIT({ noun: "cura" },
        T("Ceifa", "Toda cura crita, e o revide amedronta por 3s e dá 90 de escudo.",
          { crit: { set: 1 } },
          { "effects.4": { type: "fear", duration: 3, radius: "@radius" },
  ...
  banish: {
    ...
    effects: [
      ...
      { type: "damage_over_time", key: "banish", dps: "@dotDps", duration: "@duration",
        look: "curse", tickInterval: 0.5, color: "#6a28c8" },
    ],
    paths: {
      ...
      mastery: MASTERY({ dmg: "dotDps", noun: "dano do banimento" },
        T("Exílio", "Triplica a duração.", { duration: { mul: 3 } })),
      crit: CRIT({},
        T("Execução", "Todo tique crita, o alvo é marcado e leva um golpe ao acabar.",
          { crit: { set: 1 } },
          { "effects.2": { type: "mark", amp: 0.8, duration: "@duration" },
            "effects.1.onExpire.0": { type: "damage_instant", amount: "@dotDps*20",
              radius: 130, big: true } })),
    },
  },
