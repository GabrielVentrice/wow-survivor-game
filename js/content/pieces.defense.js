...
Object.assign(PIECES, {
  ...
  soulLeech: {
    ...
    stats: { ...CRIT_BASE, frac: 0.06, cap: 180, cooldown: 0.1 },
    ...
    paths: {
      haste: HASTE({ rate: { stat: "cooldown", verb: "Converte" } },
        T("Carapaça de Espinhos", "O escudo devolve dano em área a cada acerto.", null,
          { "effects.2": { type: "damage_instant", amount: 45, radius: 135 } })),
      mastery: MASTERY({ dmg: "frac", noun: "conversão em escudo" },
        T("Casulo", "Triplica a conversão e dobra o teto.",
          { frac: { mul: 3 }, cap: { mul: 2 } })),
      crit: CRIT({ noun: "escudo" },
        T("Imortalidade", "Todo escudo crita, e cada acerto ainda cura 12% do dano.",
          { crit: { set: 1 } }, { "effects.1": { type: "heal", frac: 0.12 } })),
    },
  },
  ...
  unendingResolve: {
    ...
    paths: {
      ...
      mastery: MASTERY({ dmg: ["shield", "cap"], noun: "escudo" },
        T("Inquebrável", "Triplica o escudo.", { shield: { mul: 3 } })),
      crit: CRIT({ noun: "escudo" },
        T("Última Palavra", "Todo gatilho crita, atordoa por 1.2s e explode em área.",
          { crit: { set: 1 } },
          { "effects.2": { type: "stun", duration: 1.2, radius: 220 },
            "effects.3": { type: "damage_instant", amount: 620, radius: 340, big: true } })),
    },
  },
  ...
  demonSkin: {
    ...
    stats: { ...CRIT_BASE, interval: 1, shield: 12, cap: 200 },
    trigger: { type: "aura", interval: "@interval" },
    effects: [
      { type: "shield", amount: "@shield", cap: "@cap",
    ...
    paths: {
      haste: HASTE({ rate: { stat: "interval", verb: "Pulsa" } },
        T("Renovação", "Cada pulso também cura 30.", null,
          { "effects.1": { type: "heal", amount: 30 } })),
      mastery: MASTERY({ dmg: ["shield", "cap"], noun: "escudo" },
        T("Placa Demoníaca", "Triplica o escudo por pulso.", { shield: { mul: 3 } })),
      crit: CRIT({ noun: "escudo" },
        T("Fortaleza", "Todo pulso crita e fere quem estiver colado.",
          { crit: { set: 1 } },
          { "effects.2": { type: "damage_instant", amount: 80, radius: 115 } })),
    },
  },
    ...
    paths: {
      ...
      mastery: MASTERY({ dmg: ["shield", "cap"], noun: "escudo" },
        T("Âncora Vital", "Cada reposição também cura 45.", null,
          { "effects.2": { type: "heal", amount: 45 } })),
      crit: CRIT({ noun: "escudo" },
        T("Legião de Almas", "Todo escudo crita, e cada reposição invoca 6 caçadores.",
          { crit: { set: 1 } },
          { "effects.3": { type: "summon", kind: "dreadstalker", ai: "chase", count: 6,
                           cap: 12, duration: 40, damage: 40 } })),
    },
  },
